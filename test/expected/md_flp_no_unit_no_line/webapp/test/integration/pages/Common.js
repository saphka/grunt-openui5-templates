sap.ui.define([
		...
		function getFrameUrl (sHash, sUrlParameters) {
			var sUrl = jQuery.sap.getResourcePath("md/flp/no/unit/no/line/app", ".html");
			sHash = sHash || "";
			sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

			if (sHash) {
				sHash = "#md_flp_no_unit_no_line-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
			} else {
				sHash = "#md_flp_no_unit_no_line-display";
			}

			return sUrl + sUrlParameters + sHash;
		}

		return Opa5.extend("md.flp.no.unit.no.line.test.integration.pages.Common", {
			...
			getMockServer : function () {
				return new Promise(function (success) {
					Opa5.getWindow().sap.ui.require(["md/flp/no/unit/no/line/localService/mockserver"], function (mockserver) {
